...
<div class="card border-0">
    <div class="card-body p-4 p-md-5">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            var container = document.getElementById('adminAddStudentForm');
            if (!container) return;

            var submitButton = container.querySelector('[data-js-submit]');

            function showSuccess(message) {
                var existing = container.querySelector('[data-student-create-alert]');
                if (existing) existing.remove();
                var alertBox = document.createElement('div');
                alertBox.className = 'alert alert-success alert-dismissible fade show mt-3';
                alertBox.setAttribute('role', 'alert');
                alertBox.setAttribute('data-student-create-alert', 'true');
            ...
            document.addEventListener('click', function (event) {
                if (!event.target.closest || !event.target.closest('#adminAddStudentForm [data-js-submit]')) return;
                ...
                xhr.onload = function () {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        showSuccess((payload && payload.message) ? payload.message : 'Student added successfully.');

                        var student = payload && payload.student ? payload.student : null;

                        if (student) {
                            var evt = new CustomEvent('admin-students:updated', { detail: { student: student } });
                            window.dispatchEvent(evt);

                            if (window.BroadcastChannel) {
                                var ch = new BroadcastChannel('admin-students-sync');
                                ch.postMessage({ type: 'student.created', student: student });
                                ch.close();
                            }
                        }
                    ...
                    showError('Unable to add the student right now.');
                };

                xhr.onerror = function () {
                    if (submitButton) submitButton.disabled = false;
                    showError('Network error while adding the student.');
                };
